...
const SalesInvoices: React.FC = () => {
  ...
  const handleDownloadPDF = () => {
    ...
    const doc = new jsPDF();
    const tableColumn = ['Invoice Name', 'Customer', 'Posting Date', 'Status', 'Grand Total'];
    const tableRows = invoices.map(inv => [inv.name, inv.customer, inv.posting_date, inv.status, inv.grand_total]);
              ...
              <table className="min-w-full bg-white/80 dark:bg-[#18181c]/80 border-2 border-[#774A67] dark:border-[#8b5cf6] rounded-2xl shadow-xl overflow-hidden">
                <thead className="bg-gradient-to-r from-[#f7f8fa] to-[#e9eaf3] dark:from-[#23232a] dark:to-[#18181c]">
                  <tr>
                    ...
                    <th className="px-6 py-4 border-b-2 border-[#774A67] dark:border-[#8b5cf6] text-lg font-bold text-[#774A67] dark:text-[#8b5cf6]">Customer</th>
                    <th className="px-6 py-4 border-b-2 border-[#774A67] dark:border-[#8b5cf6] text-lg font-bold text-[#774A67] dark:text-[#8b5cf6]">Posting Date</th>
                ...
                <tbody>
                  {invoices.length === 0 ? (
                    ...
                    invoices.map((inv, idx) => (
                      ...
                      >
                        <td className="px-6 py-4 border-b border-[#774A67] dark:border-[#8b5cf6] font-semibold text-gray-900 dark:text-white">{inv.name}</td>
                        <td className="px-6 py-4 border-b border-[#774A67] dark:border-[#8b5cf6] text-gray-700 dark:text-gray-300">{inv.customer}</td>
                        <td className="px-6 py-4 border-b border-[#774A67] dark:border-[#8b5cf6] text-gray-700 dark:text-gray-300">{inv.posting_date}</td>
                        <td className="px-6 py-4 border-b border-[#774A67] dark:border-[#8b5cf6]">
                          <span className={`px-3 py-1 rounded-full text-xs font-bold ${inv.status === 'Unpaid' ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' : 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400'}`}>{inv.status}</span>
                        </td>
                        <td className="px-6 py-4 border-b border-[#774A67] dark:border-[#8b5cf6] text-gray-900 dark:text-white font-bold">{inv.grand_total}</td>
                      </motion.tr>
                    ))
                  )}
                </tbody>
